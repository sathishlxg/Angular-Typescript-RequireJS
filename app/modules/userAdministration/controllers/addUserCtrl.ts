...
class AddUserCtrl implements IAddUserCtrl {
    ...
    constructor(public user: models.User, private service: service.UserService, private $state: ng.ui.IStateService) {
        this.user = user;
    }

    public save(): void {
        this.service.saveUser(this.user)
            .then((data) => { this.$state.get("userSummary"); })
            .catch((reason) => { alert("an error occured while saving the user"); });
    }

    public delete(): void {
        this.service.deleteUser(1)
            .then((data) => { this.$state.get("userSummary"); })
            .catch((reason) => { alert("an error occured"); });;
    }
}
